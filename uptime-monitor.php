...
function uptime_monitor_settings_page() {
    ...
    echo '<td>';
    echo '<input type="url" id="server_health_report_url" name="server_health_report_url" value="' . esc_attr($server_health_report_url) . '" class="regular-text" placeholder="https://example.com/private-health/latest.php">';
    echo '<p class="description">Optional. URL for the private server-health-report JSON endpoint.</p>';
    echo '</td>';
    echo '</tr>';

    echo '<tr>';
    echo '<th scope="row"><label for="server_health_report_auth_header">Server Health Request Header</label></th>';
    echo '<td>';
...
function uptime_monitor_parse_server_health_report_header($configured_header) {
    ...
    if ($colon_pos !== false && $colon_pos > 0) {
        $name = trim(substr($configured_header, 0, $colon_pos));
